...
from utils import COLOR_ORANGE, LOWER_OPACITY, StateCounter, bash, code, dimmed_list_item, \
    generate_qr_code, \
    iterate_grid, show, HideRest, ShowRest, code_step
...
@slides.slide()
def intro(slide: Box):
    slide.box(width=350).image("images/python-pizza.png")
    slide.box(p_bottom=sh(40)).text("Tips for better tests", style=T(bold=True))
...
def whoami(slide: Box):
    ...
    lst.item().text("Lecturer @ VSB-TUO")
...
def why_tests(slide: Box):
    ...
    # slide.box(show="6+", p_top=40).text("Code without (useful) tests => legacy code",
    #                                     escape_char="#")


@slides.slide()
def why_tests_suck(slide: Box):
    row = slide.box(p_bottom=100, horizontal=True)
    row.box().text("Tests kinda suck", T(size=80))
    row.box(width=20)
    row.box(width=80).image("images/disappointed.svg")
    lst = unordered_list(slide.box())
    items = [
...
@slides.slide()
def how_to_better_tests(slide: Box):
    slide.box().text("Can we make tests less annoying?")
    slide.box(p_top=100, show="next+").text("Use-case: GitHub merge queue bot", T(size=56))
...
#     state.tip(slide, "Build high-level test APIs")
...
# )
#
# ctx.approve(Approval("r1").pr(2)) # builder pattern
# ctx.approve("r1")                 # "r1" => Approval("r1")
# ctx.approve("r1", ("a/b", 2))     # ("a/b", 2) => PR(2).repo("a/b")
# """, [
#         show(1) + [HideRest],
#         show(2) + [HideRest],
#         show(8) + [HideRest],
...
#
#     code_step(slide.fbox(), """
# repo.approve_pr(1, User("karel"))
# assert repo.get_pr(1).get_approval().is_approved()
# assert repo.get_pr(1).get_approval().approver().name() == "karel"
# assert repo.get_pr(1).get_priority() == 0
#
# vs
#
# ctx.get_pr(1)\\
...
def outro(slide: Box):
    ...
    qr = generate_qr_code(
        "https://github.com/kobzol/talks/blob/main/2026/ostrava-python-pizza/tips-for-better-tests",
        scale=14)
    slide.box().image(qr, image_type="png")
    ...
    if PRODUCTION_BUILD:
        ...
        slide.box().text(f"in {lines} lines of Python",
                         style=T(size=40))
