...
@slides.slide(bg_color=BG_PURPLE)
def intro(slide: Box):
    slide.update_style("default", T(color="#FFFFFF"))
    slide.box(x="[6%]", y="[6%]").image("images/meetupdate-logo.png")
    row = slide.box(horizontal=True, p_bottom=sh(60))
    row.box(width=sw(400), p_right=sw(60)).image("images/rust-logo-white.png")
    row.box(width=sw(400), p_right=sw(60)).image("images/ferris-cz.png")
    slide.box(p_bottom=sh(40)).text("Tipy, jak psát lepší (Rust) testy", style=T(bold=True))
...
def whoami(slide: Box):
    ...
    lst.item().text("Researcher @ IT4Innovations")
...
def bors(slide: Box):
    ...
    slide.box(p_top=60).text("GitHub merge queue bot")


@slides.slide()
def bors_overview_1(slide: Box):
    slide.box(width=1400).image("images/bors-overview-1.png")


@slides.slide()
def bors_overview_2(slide: Box):
    slide.box(width=1600).image("images/bors-overview-2.png")
