...
GITHUB_BG_COLOR = "#0D1117"
INTELLIJ_BG_COLOR = "#1E1F22"
HACKMD_BG_COLOR = "#27272A"
...
def quotation(box: Box, quotation: str, source: Optional[str] = None, **text_args) -> BoxItem:
    ...
    inner = wrapper.box(width="100%", padding=sw(30), x="[98%]")
    ...
    inner.box(width="100%", p_top=quote_width).text(quotation, style=T(**text_args))
    source = source or ""
...
def dimmed_list_item(parent: ListBuilder, text: str, show: int,
                     highlight_steps=0, last=False, **text_style) -> TextBoxItem:
    if not last:
        wrapper = parent.item(show=f"{show}+")
        wrapper.box(show=f"{show + highlight_steps + 1}+").text(text, T(opacity=LOWER_OPACITY))
        return wrapper.overlay(show=f"{show}-{(show + highlight_steps)}").text(text, T(**text_style))
    else:
        return parent.item(show=f"{show}+").text(text, **text_style)


def dimmed_list_items(parent: ListBuilder, show: int, items: List[str], dim_last=False,
                      **text_style) -> List[TextBoxItem]:
    boxes = []
    for (index, item) in enumerate(items):
        box = dimmed_list_item(parent, item, show=show + index,
                         last=index == len(items) - 1 if not dim_last else False,
                         **text_style)
        boxes.append(box)
    return boxes
...
SHARED_CHAPTER_COUNTER = 0


def chapter(slide: Box, name: str, chapter: Optional[int] = None):
    global SHARED_CHAPTER_COUNTER

    if chapter is None:
        SHARED_CHAPTER_COUNTER += 1
        chapter = SHARED_CHAPTER_COUNTER

    col = slide.box()
    col.box(p_bottom=40).text(f"Chapter {chapter}", T(size=80))
    col.box().text(name, T(size=100))


SHARED_BORS_IMPL_COUNTER = 0


def bors_impl_counter(slide: Box, counter: Optional[int] = None, show: Optional[str] = "last+"):
    global SHARED_BORS_IMPL_COUNTER

    if counter is None:
        SHARED_BORS_IMPL_COUNTER += 1
        counter = SHARED_BORS_IMPL_COUNTER

    radius = 20
    box = slide.box(y="[98%]", x="[98%]", show=show).rect(bg_color="#116466", rx=radius, ry=radius)
    line = box.box(horizontal=True, p_y=15, p_x=25)
    line.box(p_right=10).text(f"Bors implementation counter:", T(color="white"))
    line.box(width=50).text(f"~bold{{{counter}}}", T(color="white"))


def bors_impl(box: Box, name: str, date: str, icon: str, lines: str, stroke_width: int = 4):
    height = 50
    wrapper = box.box(width=800)
    radius = 20
    wrapper.rect(color="black", stroke_width=stroke_width, rx=radius, ry=radius)
    wrapper = wrapper.box(padding=50)
...
class Timeline:
    def __init__(self, slides: Slides, start: int, end: int, years: List[float] = None):
        self.slides = slides
        self.start = start
        self.end = end
        self.years = years or []
    ...
    def clone(self) -> "Timeline":
        return Timeline(slides=self.slides, start=self.start, end=self.end, years=list(self.years))

    def render_year(self, year: float, render_fn: Callable[[Box], None]):
        slide = self.slides.new_slide(debug_boxes=False)

        timeline_box = slide.fbox(height=140, p_top=40)
        timeline = timeline_box.box(width="90%", height=30)
        timeline.rect(bg_color="black")

        # Year ticks
        for y in self.years:
            self._render_year_tick(timeline, y)
        entry_tick = self._render_year_tick(timeline, year)
        entry_tick.overlay().rect(bg_color=COLOR_ORANGE)

        # Margin
        slide.sbox(height=60)
        content_box = slide.box(width="100%", height="fill")

        render_fn(content_box)
        self.years.append(year)

    def _render_year_tick(self, timeline: Box, year: float) -> Box:
        this_year = self.end
        first_year = self.start
        ...
        timeline_entry.box(p_top=20).text(str(year), style=T(size=44), rotation=-45)

        return entry_tick


def text_box(parent, text: str, padding: int = 20, **text_args) -> TextBoxItem:
    return parent.rect(color="black", stroke_width=4).box(padding=padding).text(text,
                                                                                T(**text_args))
...
def arrow_box(box: Box, text: Optional[str] = None, both_sides: bool = False, **text_args):
    ...
    if text is not None:
        box.box(x=box.x("0"), y=box.y("100%").add(30), width=box.layout.width_definition).text(text,
                                                                                               T(**text_args))


def source(slide: Box, name: str):
    box = slide.box(x="[98%]", y="[98%]")
    box.text(f"Source: {name}", T(size=40))
