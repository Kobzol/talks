...
""", [
    ...
    def finish_something(slide: Box):
        ...
        lst.item(show="6+").text("Use coverage marks", "small")

        width = 1700
        code_step(slide.fbox(show="4-5", y=200), """
async fn unapprove() {
    …
    bors.post_comment("@bors r-").await?;

    // Bot comment acts as a synchronization point
    insta::assert_snapshot!(
        bors.get_next_comment().await?,
        @"Commit pr-1-sha has been unapproved."
    );

    bors.get_pr().await.expect_unapproved();
}
""", [
            show(3) + skip(7) + last(2),
            [ShowRest]
        ], show_start=4, width=width)
        project(slide, "bors", show="4-5")
...
async fn test() {
    bors.start_workflow().await;
    WAIT_FOR_WORKFLOW_STARTED.sync().await;
    bors.assert_workflow_started().await;
}
""", [
            show(2) + [HideRest],
            show(13) + [HideRest],
...
where
    F: AsyncFnOnce(&mut Ctx) -> anyhow::Result<()>
{
...
""", [
    ...
    def inspiration(slide: Box):
        slide.box().text("Some further inspiration:")

        lst = unordered_list(slide.box(p_top=40))
        lst.item().text("tokio turmoil", T(font="Ubuntu Mono"))
        lst.item(show="next+").text("Deterministic Simulation Testing")
        lst2 = lst.ul()
        lst2.item().text("TigerBeetle")
        lst2.item().text("antithesis")
